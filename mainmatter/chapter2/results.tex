...
The interweaving of distal CREs with target gene promoters establishes regulatory plexuses with some to be ascribed to specific genes 125,165.
Regulatory plexuses stem from chromatin interactions orchestrated by various factors including ZNF143, YY1, CTCF and the cohesin complex 141–143.
...
We next inferred the regulatory plexus of FOXA1 using the Cross Cell-Type Correlation based on DNA accessibility (C3D) method 311.
C3D aggregates and draws correlation of DHS signal intensities between the cell line of choice and the DHS signal across all systems in the database 311.
...
To delineate the CREs that could be actively involved in the transcriptional regulation of FOXA1, we annotated the DHS with available ChIP-seq data for histone modifications and TFs conducted in LNCaP, 22Rv1, VCaP prostate cancer cell lines and primary prostate tumours (Figure 3.2b) 186,232.
Close to 60\% (33/55) of the putative FOXA1 plexus CREs are positively marked by H3K27ac profiled in primary prostate tumours 232, indicative of active CREs in tumours (Figure 3.2b) 140.
Next, considering that noncoding SNVs can target a set of CREs that converge on the same target gene in cancer 125, we overlapped the somatic SNVs called from the whole-genome sequencing across 200 primary prostate tumours to the 33 H3K27ac-marked DHS predicted to regulate FOXA1 214,234.
...
Expanding on the idea that multiple CREs can converge to regulate the expression of a single target gene 125,165,313, we asked whether the CREs we identified collaboratively regulate FOXA1 mRNA expression.
...
SNVs can alter the transactivation potential of CREs 125,127,145,147,155,314–318.
...
We next assessed if the changes in transactivation potential induced by noncoding SNVs related to changes in TF binding to CREs by allele-specific ChIP-qPCR 125,155,315 in LNCaP prostate cancer cells.
