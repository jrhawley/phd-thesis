...
\newlongtable{l l l l}{Random variables used throughout \Cref*{chap:JS_appendix} and \Cref*{chap:JS}}{
  ...
  \gls{ols} mean estimator  & $\hat{\mu}^{(OLS)} $ & None & Estimator of mean fold change using the \gls{ols} method. \\
  \gls{js} mean estimator   & $\hat{\mu}^{(JS)}$  & None & Estimator of mean fold change using the \gls{js} method. \\
}
...
The goal of the differential analysis is to estimate the $|S| \times p$ coefficients in $\beta_s \forall s \in S$, and to determine which coefficients differ significantly from 0.
