...
The samples stored at $-80$ \textdegree C were thawed on ice and spun down at $300 \times g$ for 5 min at 4 \textdegree C.
...
Next, an equal volume ($\sim$ 325 $\mu$L) of phenol-chloroform-isoamyl alcohol mixture (25:24:1) (Sigma) was also added to the column.
The column was then inverted for thorough mixing and spun down at $17 000 \times g$ for 5 min at 4 \textdegree C.
The surface layer on top of the gel upon spinning contains the sample and is transferred to a clean 1.5 mL tube ($\sim$ 325 $\mu$L).
...
The Hi-C libraries for each tumour sample were sent for shallow paired-end 150 bp sequencing ($\sim$ $10-15 \times 10^6$ reads per sample) on an Illumina NextSeq 500 at the Princess Margaret Genomics Centre.
...
\subsubsection{\Glsfmtshort{tad} identification}
...
\subsubsection{Sample clustering by \glsfmtshortpl{tad}}
...
Breakpoint pairs for each patient were called on the merged \gls{bam} files \cite{liSequenceAlignmentMap2009} using $\texttt{hic\_breakfinder}$ (commit 30a0dcc6d01859797d7c263df7335fd2f52df7b8) \cite{dixonIntegrativeDetectionAnalysis2018}.
...
\subsubsection{Determination of \glsfmtshort{sv} breakpoints altering intra-\glsfmtshortpl{tad} contacts}
...
Contact frequencies from the \gls{ice}-normalized, 20 kbp contact matrices were extracted, with the rows as the bins containing the anchor and the columns as the target regions (the x-axes in \Cref{fig:3D_fig6}e).
...
\subsubsection{Complex structural variant assembly}
...
\subsection{Patient tumour tissue H3K27ac \glsfmtshort{chipseq}}
...
Unique peak calls and de-duplicated pull-down and control \gls{bam} files from tumour samples were loaded into R with the DiffBind package (v2.14.0) \cite{starkDiffBindDifferentialBinding2011} using DESeq2 (v1.26.0) \cite{loveModeratedEstimationFold2014} as the differential analysis model.
...
\subsubsection{Tumour sample \glsfmtlong{rnaseq}}
...
\subsubsection{\glsfmtlong{rnaseq} data pre-processing}
